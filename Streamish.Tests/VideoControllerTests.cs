...
namespace Streamish.Tests
{
    public class VideoControllerTests
    {
        ...
        public void Search_Method_Returns_Videos_That_Fit_Criteria()
        {
            ...
            var controller = new VideoController(repo);
            var testString = "20";
            var testString2Results = "Description 2";
            var testBool = true;

            // act
            var result = controller.Search(testString, testBool);
            var result2Values = controller.Search(testString2Results, testBool);

            // assert
            var okResult = Assert.IsType<OkObjectResult>(result);
            var okResult2 = Assert.IsType<OkObjectResult>(result2Values);
            var actualVideos = Assert.IsType<List<Video>>(okResult.Value);
            var twoVideos = Assert.IsType<List<Video>>(okResult2.Value);
            
            Assert.Single(actualVideos);
            Assert.Equal(2, twoVideos.Count);
